#!/bin/sh
# Install the spec-driven + TLA+ sub-agent workflow (parallel implementation)
# into a target project.
# Run from the target project root. Idempotent: re-running skips what exists.
# Usage: ~/.claude/skills/spec-driven-tla-parallel/install.sh [target-dir]
set -eu

SKILL_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
TARGET=${1:-$PWD}
cd "$TARGET"
echo "== spec-driven-tla-parallel → $TARGET =="
